(ns ecwid.main
  "Kotodama WASM entrypoint for the Ecwid clean-room actor (L4) — Clojure port.
  ...
  facts (`ecwid.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "Product"   :plural "products"   :id-prefix "ecwid_pro"
   ...
   {:entity "Customer"  :plural "customers"  :id-prefix "ecwid_cus"
    :fields [:email :firstName :lastName :phone] :required [:email :firstName]
    :coerce {}                       :refs {}}
   {:entity "Collection" :plural "collections" :id-prefix "ecwid_col"
...
(defn emit-facts
  "EAVT facts for one record: {\"ecwid.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "ecwid-compat" :tier tier :entities entities} 200])
